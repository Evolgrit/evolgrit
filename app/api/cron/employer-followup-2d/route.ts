...
export async function GET(req: Request) {
  ...
  return NextResponse.json(
    {
      ...
      debug: {
        ...
        match: Boolean(secret) && auth === `Bearer ${secret}`,
      },
    },
    { status: 401 }
  );

  // --- REAL LOGIC (will run after we remove the debug return) ---
  /*
  ...
  for (const r of data ?? []) {
    await resend.emails.send({
      ...
      html: `<div style="font-family: ui-sans-serif, system-ui; line-height: 1.6;">
        <p>Hi,</p>
        <p>Two quick questions so we can suggest the right pilot setup:</p>
        <ul>
