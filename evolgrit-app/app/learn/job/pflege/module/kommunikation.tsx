...
const moduleData = require("../../../../../content/job/pflege/module_kommunikation_01.json");

export default function PflegeModuleKommunikationScreen() {
  const router = useRouter();
  const insets = useSafeAreaInsets();

  return (
    <ScreenShell title="Pflege · Kommunikation" showBack>
          ...
          <YStack gap="$3">
            {(moduleData.liveDialogues ?? []).map((dialogueId: string) => {
              const labelMap: Record<string, string> = {
                job_pflege_live_10_deeskalation_patient: "Deeskalation (Patient)",
                job_pflege_live_11_deeskalation_angehoerige: "Deeskalation (Angehörige)",
                job_pflege_live_12_erklaeren_wartezeit: "Wartezeit erklären",
              };
              return (
                <Pressable
                  key={dialogueId}
